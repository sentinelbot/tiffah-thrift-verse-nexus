...
import { ScanBarcode, Package, Check, X } from 'lucide-react';
...
const ProductScanner = () => {
  ...
  const startScanning = async () => {
    ...
    setIsScanning(true);
    toast.info('Scanning for products...');
    
    // Simulate a scan after a short delay
    setTimeout(() => {
      const barcode = 'TTS-PROD-' + Math.floor(Math.random() * 1000).toString().padStart(3, '0');
      setLastScan(barcode);
      setIsScanning(false);
